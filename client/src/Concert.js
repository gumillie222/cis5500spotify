...
export default class Map extends React.Component {
   ...
   handlePinClick = (concert) => {
    ...
    componentDidMount = () => {
        // First, make the Axios call to get the concerts data.
        axios.get("http://localhost:8081/latitudelongitude")
            .then(res => {
                // Here, we get concert data successfully.
                // We then check for the user's current position.
                navigator.geolocation.getCurrentPosition(
                    (position) => {
                        console.log(position.coords);
                        // Now set state for both fetched concert data and position data.
                        this.setState({
                            concerts: res.data,
                    ...
                    (error) => {
                        ...
                        // Even if there's a geolocation error, set the concerts data.
                        this.setState({
                            concerts: res.data
                        });
                    }
            ...
            .catch(error => {
                console.log("Error Getting Concert Data: " + error.message);
                // Handle failure where we might not get concert data.
            });
    }
                        ...
                        if (this.state.selectedConcertId === concert.id) {
                            return (
                                <div 
                                    ...
                                    lat={concert.latitude}
                                    lng={concert.longitude}
                                    style={{ backgroundColor: "#E9D6D7", width: 100, transform: 'translate(-50%, -100%)', padding: 10, borderRadius: 20}}>
                                    <Typography style={{textAlign: "center"}}>{concert.name}</Typography>
                                </div>
                            );
                        }
                        return null;  // This ensures no other concert info is rendered
                    })}
                </GoogleMapReact>
            </div>
        );
